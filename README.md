...
页面顶部有一个状态灯(`AI 兜底:在线/离线/缺少 Key`),可以直接看出后端服务是否连上。
...
- `app.js` — 本地规则解析与计算逻辑(基于 [math.js](https://mathjs.org/) 做表达式求值),以及调用后端兜底服务的逻辑
- `server/` — 可选的兜底后端服务(Node.js + Express),调用通义千问(Qwen)API
  - `server.js` — 服务入口,提供 `/api/evaluate` 和 `/api/health` 两个接口
