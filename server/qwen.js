/**
 * Shared AI-fallback logic (Qwen/DashScope), with no framework dependency
 * (no express/cors) so it can be required both by:
 *   - server/server.js — a local Express server for `npm run` dev, and
...
// Calls DashScope with the built prompt and returns { result, resultType, raw }.
// Throws an Error with a `.status` (HTTP status to relay) on failure.
async function evaluateWithQwen(lines, lineIndex) {
  const apiKey = process.env.QWEN_API_KEY;
  if (!apiKey) {
    const err = new Error("Server is missing QWEN_API_KEY");
    err.status = 500;
    throw err;
  }
  ...
  // (Hobby plan) so we return a clean JSON timeout error instead of the
  // platform hard-killing the function mid-request.
  const controller = new AbortController();
  const timeout = setTimeout(() => controller.abort(), 9_000);

  let r;
  try {
    r = await fetch(`${DASHSCOPE_BASE}/chat/completions`, {
      method: "POST",
      headers: {
    ...
    const err = new Error(isAbort ? "Upstream timeout" : "Request failed");
    err.status = isAbort ? 504 : 500;
    throw err;
  } finally {
